...
def load_prompt_string_from_file(prompt_path_str: str):
    ...
    if not os.path.isfile(prompt_file_path):
        raise FileNotFoundError(prompt_file_path + ' does not match any existing files.')
...
def main(args: Namespace) -> None:
    ...
    print('Using {device=} and {device_map=}')
    ...
    # Load HF Model
    print('Loading HF model with dtype={model_dtype}...')
    try:
    ...
    if args.autocast_dtype is not None:
        ...
        print('Using autocast with dtype={autocast_dtype}...')
    else:
    ...
    for temp, topp, topk, repp, nrnz, seed in itertools.product(
        ...
        for batch in batches:
            ...
            print('{' + bs + '=}, { '+ input_tokens+ ' =}, {'+ output_tokens +'=}')
            print('{'+ total_input_tokens  +' =}, {'+ total_output_tokens +'=}')
            print('{encode_latency=:.2f}ms, {gen_latency=:.2f}ms, {decode_latency=:.2f}ms, {total_latency=:.2f}ms'
            )
            print('{' + str(latency_per_output_token) + '=:.2f}ms/tok')
            print('{output_tok_per_sec=:.2f}tok/sec')
